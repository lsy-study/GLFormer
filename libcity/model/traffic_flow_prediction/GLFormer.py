...
class DataEmbedding(nn.Module):
    def __init__(
        ...
        self.weight_daytime = nn.Parameter(torch.ones(1)) if self.add_time_in_day else None
        self.weight_weekday = nn.Parameter(torch.ones(1)) if self.add_day_in_week else None
        self.weight_spatial = nn.Parameter(torch.ones(1))
        self.norm = nn.LayerNorm(embed_dim)
        self.dropout = nn.Dropout(drop)

    def forward(self, x, lap_mx):
        origin_x = x
        batch_size, time_steps, node_num, _ = origin_x.shape
        emb_list = []
        value_emb = self.value_embedding(origin_x[:, :, :, :self.feature_dim])
        emb_list.append(self.weight_value * value_emb)
        position_emb = self.position_encoding(value_emb)
        emb_list.append(self.weight_position * position_emb)
        if self.add_time_in_day:
            time_in_day = (origin_x[:, :, :, self.feature_dim] * self.minute_size).round().long()
            daytime_emb = self.daytime_embedding(time_in_day)
            emb_list.append(self.weight_daytime * daytime_emb)
        if self.add_day_in_week:
            weekday_emb = self.weekday_embedding(
                origin_x[:, :, :, self.feature_dim + 1: self.feature_dim + 8].argmax(dim=3)
            )
            emb_list.append(self.weight_weekday * weekday_emb)
        spatial_emb = self.spatial_embedding(lap_mx)
        spatial_emb = spatial_emb.unsqueeze(0).unsqueeze(0)
        spatial_emb = spatial_emb.expand(batch_size, time_steps, -1, -1)
        emb_list.append(self.weight_spatial * spatial_emb)
        x = sum(emb_list)
        x = self.norm(x)
        x = self.dropout(x)
...
class TemporalSelfAttention(nn.Module):
    def __init__(
            ...
            attn_drop=0., proj_drop=0., device=torch.device('cpu'), layerdrop_prob=0.03
    ):
        super().__init__()
        assert dim % t_num_heads == 0
        self.t_num_heads = t_num_heads
        self.head_dim = dim_out // t_num_heads
        self.scale = self.head_dim ** -0.5
        self.device = device
        self.t_attn_size = t_attn_size

        self.t_q_conv = nn.Conv2d(dim, 16, kernel_size=1, bias=qkv_bias)
        self.t_k_conv = nn.Conv2d(dim, 16, kernel_size=1, bias=qkv_bias)
        self.t_v_conv = nn.Conv2d(dim, 16, kernel_size=1, bias=qkv_bias)

        self.global_conv = nn.Conv2d(dim, 16, kernel_size=(t_attn_size, 1), bias=qkv_bias)
    ...
    def forward(self, x):
        B, T, N, D = x.shape
        def apply_layerdrop(layer_output):
            if torch.rand(1).item() < self.layerdrop_prob:
                return torch.zeros_like(layer_output)
            return layer_output


        t_q = apply_layerdrop(self.t_q_conv(x.permute(0, 3, 1, 2)).permute(0, 3, 2, 1))
        t_k = apply_layerdrop(self.t_k_conv(x.permute(0, 3, 1, 2)).permute(0, 3, 2, 1))
        t_v = apply_layerdrop(self.t_v_conv(x.permute(0, 3, 1, 2)).permute(0, 3, 2, 1))
        ...
        t_local_x = (t_attn @ t_v).transpose(2, 3)


        global_x = self.global_conv(x.permute(0, 3, 1, 2))
        global_x = global_x.squeeze(2).permute(0, 2, 1)


        t_x = t_local_x.reshape(B, T, N, self.t_num_heads * self.head_dim) + global_x.unsqueeze(1)
...
class STSelfAttention(nn.Module):
    def __init__(
        self, dim, s_attn_size, t_attn_size,
        ...
        assert dim % total_heads == 0, "dim must be divisible by total heads"
        ...
        # 保存比例
        self.geo_ratio = geo_num_heads / total_heads
        self.sem_ratio = sem_num_heads / total_heads
        self.t_ratio = t_num_heads / total_heads
        ...
        # 局部空间 QKV via conv
        self.geo_q_conv = nn.Conv2d(dim, int(dim * self.geo_ratio), 1, bias=qkv_bias)
        self.geo_k_conv = nn.Conv2d(dim, int(dim * self.geo_ratio), 1, bias=qkv_bias)
        self.geo_v_conv = nn.Conv2d(dim, int(dim * self.geo_ratio), 1, bias=qkv_bias)
        self.geo_attn_drop = nn.Dropout(attn_drop)

        # 语义空间 QKV via conv
        self.sem_q_conv = nn.Conv2d(dim, int(dim * self.sem_ratio), 1, bias=qkv_bias)
        self.sem_k_conv = nn.Conv2d(dim, int(dim * self.sem_ratio), 1, bias=qkv_bias)
        self.sem_v_conv = nn.Conv2d(dim, int(dim * self.sem_ratio), 1, bias=qkv_bias)
        self.sem_attn_drop = nn.Dropout(attn_drop)

        # 时间 QKV via conv
        self.t_q_conv = nn.Conv2d(dim, int(dim * self.t_ratio), 1, bias=qkv_bias)
        self.t_k_conv = nn.Conv2d(dim, int(dim * self.t_ratio), 1, bias=qkv_bias)
        self.t_v_conv = nn.Conv2d(dim, int(dim * self.t_ratio), 1, bias=qkv_bias)
        self.t_attn_drop = nn.Dropout(attn_drop)

        # 改进点：first fusion MLP, 输入 sem 和 t 特征
        sem_t_dim = int(dim * (self.sem_ratio + self.t_ratio))
        self.fuse_mlp = nn.Sequential(
            nn.Linear(sem_t_dim, sem_t_dim),
            nn.ReLU(),
            nn.Linear(sem_t_dim, sem_t_dim)
        )
        ...
        # 全局信息提取卷积
        self.global_conv = nn.Conv2d(dim, 16, (t_attn_size, 1), bias=qkv_bias)

        # 时间自注意力模块
        self.model = TemporalSelfAttention(dim, s_attn_size, t_attn_size)

    def forward(self, x, x_patterns, pattern_keys, geo_mask=None, sem_mask=None):
        B, T, N, D = x.shape

        # LayerDrop helper
        def apply_layerdrop(layer_out):
            if torch.rand(1).item() < self.layerdrop_prob:
                return torch.zeros_like(layer_out)
            return layer_out

        # 时间注意力
        t_x = apply_layerdrop(self.model(x))  # B,T,N,d_t

        # 语义注意力
        sem_q = apply_layerdrop(self.sem_q_conv(x.permute(0,3,1,2))).permute(0,2,3,1)
        sem_k = apply_layerdrop(self.sem_k_conv(x.permute(0,3,1,2))).permute(0,2,3,1)
        sem_v = apply_layerdrop(self.sem_v_conv(x.permute(0,3,1,2))).permute(0,2,3,1)
        ...
        sem_attn = sem_attn.softmax(-1)
        sem_attn = self.sem_attn_drop(sem_attn)
        sem_x = (sem_attn @ sem_v).transpose(2,3).reshape(B, T, N, int(D * self.sem_ratio))

        # 第一层拼接融合：sem_x 与 t_x
        sem_t_cat = torch.cat([sem_x, t_x], dim=-1)  # B,T,N,sem_t_dim
        sem_t_x = self.fuse_mlp(sem_t_cat)           # B,T,N,sem_t_dim
        # 局部空间注意力
        geo_q = apply_layerdrop(self.geo_q_conv(x.permute(0,3,1,2))).permute(0,2,3,1)
        geo_k = apply_layerdrop(self.geo_k_conv(x.permute(0,3,1,2))).permute(0,2,3,1)
        geo_v = apply_layerdrop(self.geo_v_conv(x.permute(0,3,1,2))).permute(0,2,3,1)
        ...
        geo_attn = geo_attn.softmax(-1)
        geo_attn = self.geo_attn_drop(geo_attn)
        geo_x = (geo_attn @ geo_v).transpose(2,3).reshape(B, T, N, int(D * self.geo_ratio))
        # 第二层拼接融合：sem_t_x 与 geo_x
        fusion = torch.cat([sem_t_x, geo_x], dim=-1)  # B,T,N,D
        x_out = self.proj(fusion)
...
class GLFormer(AbstractTrafficStateModel):
